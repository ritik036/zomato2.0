...
function MenuItems({ items, onItemDeleted, isSeller }: MenuItemsProps) {
  ...
  const handleDelete = async (itemId: string) => {
    const confirm = window.confirm("Sure to delete this item ?");
    if (!confirm) {
      return;
    }
    try {
  ...
  const { fetchCart, setQuantity } = useAppData();
  const addToCart = async (restaurantId: string, itemId: string) => {
    try {
      ...
      toast.success(data.message);
      fetchCart();
      setQuantity(data?.cart?.quantity || 0);
    } catch (error: any) {
          ...
          return (
            ...
            >
              <div className="relative shrink-0">
                ...
                />
                {!item.isAvailable && (
                  <span className="absolute inset-0 flex items-center justify-center rounded bg-black/60 text-sx font-semibold text-white">
                    Not Available
                  </span>
                )}
              </div>
                ...
                <div className="flex items-center justify-between ">
                  ...
                  {isSeller && (
                    <div className="flex gap-2">
                      ...
                      <button
                        className="rounded-lg p-2 text-red-500 hover:bg-red-500 "
                        onClick={() => {
                  ...
                  {!isSeller && (
                    ...
                    >
                      {isLoading ? (
                        <VscLoading className="h6 w-6 animate-spin " />
                      ) : (
                        <BsCartPlus size={18} />
                      )}
                    </button>
